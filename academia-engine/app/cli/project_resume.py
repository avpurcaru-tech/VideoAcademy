...
def main() -> int:
    ...
    parser.add_argument("--timeout",type=float,default=900); parser.add_argument("--confirm",action="store_true"); args=parser.parse_args()
    try:
        ...
        record=ProjectResumeService(generation,registry).resume(args.project_id,
            ...
            MusicPollingPolicy(interval_seconds=args.interval,timeout_seconds=args.timeout),confirmed=args.confirm)
    except Exception:
        print("Project resume failed at a safe orchestration boundary.")
